...
class Ray:
    ...
    def cast(self, boundary: Boundary) -> Union[np.ndarray, None]:
        ...
        Returns:
            Union[np.ndarray, None]: The intersection point as a numpy array of shape (2, ), if there is
                an intersection, None otherwise.
        """
...
class Car:
    def __init__(
        ...
        Args:
            ...
            rotation (float, optional): The initial rotation of the car in degrees. Defaults to 0.0,
                which is facing right.
        ...
        self.__image_width = 24
        self.__image_height = 54
    ...
    def get_passed_gate(self, gates: List[RewardGate]) -> Union[RewardGate, None]:
        """
        Check if the car has passed through an active reward gate.

        Args:
            gates (List[RewardGate]): A list of reward gates to check for intersection.

        Returns:
            Union[RewardGate, None]: The reward gate that the car has passed through,
                or None if no gate has been passed.
        """
        for gate in gates:
...
class CarEnv(gym.Env):
    ...
    def __init__(
        ...
        # Attributes that will be initialized in the reset method
        self.__next_gate_index = None
        self.__remaining_reward_gates = None
        self.__passed_reward_gates = None
        self.__total_reward_gates = None
        self.__reward_gates = None
        self.__boundaries = None
        self.__reward_gates_points = None
        self.__inner_track_points = None
        self.__outer_track_points = None
        self.__track_data = None

        self.__car = Car(
            0,
            0,
            image="lib/assets/car.png",
        )
    ...
    def load_track(self, track_path: str) -> dict:
        ...
        Args:
            track_path (str): The path to the track file.
    ...
    def _get_obs(self):
        ...
        # Get the car's position, velocity, and rotation as the first four observations and normalize them
        obs = [self.__car.get_position()[0] / self.__width,
               self.__car.get_position()[1] / self.__height,
    ...
    def reset(self, seed=None, options=None):
        """
        Resets the environment to its initial state. Possible options include setting "track_path" to the path of
        the track file as a string.
        ...
        # If the track data is not loaded, load it
        if self.__track_data is None:
            try:
                self.__track_data = self.load_track(self.__current_track_path)
            except FileNotFoundError:
                print(f"Track file not found at {self.__current_track_path}.")
        ...
        for i, (g_start, g_end) in enumerate(
            ...
            self.__reward_gates.append(
                RewardGate(g_start[0], g_start[1], g_end[0], g_end[1], i)
            )
    ...
    def step(self, action):
        ...
        if action == 0:
            ...
            reward += 0.01  # reward for moving forward
        ...
        if self.__car.is_destroyed():
            terminated = True
            reward -= 3.0  # penalty for collision
...
def register_env():
    gym.register(id="CarEnv-v0", entry_point="lib.car_env:CarEnv")
